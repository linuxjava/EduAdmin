<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="秒杀课程" min-width="150px" align="center">
        <template slot-scope="{row}">
          <div style="display: flex">
            ...
            <div
              ...
              <span style="color: red;font-size: 12px">秒杀价格：{{ row.price }}</span>
            </div>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="已使用人数" width="250px" align="center">
        <template slot-scope="{row}">
          {{ row.used_num }}
        </template>
      </el-table-column>

      <el-table-column label="秒杀状态" width="250px" align="center">
        <template slot-scope="{row}">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        startEndTime: [
          {required: true, message: '请选择秒杀开始和结束时间', trigger: 'change'},
        ]
      },
    }
  },
  ...
  methods: {
    ...
    async getList() {
      ...
      const res = await fetchSplash(this.listQuery)
    ...
    async commit(){
      ...
      if(this.dialogType === 'add') {
        ...
        const res = await splashEdit(this.form)
        this.dialogVisible = false
        if(res.code === 20000) {
